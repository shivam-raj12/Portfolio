...
import { BookOpen, Calendar, Clock, Tag, Search, Filter, ArrowLeft, Eye, Heart } from 'lucide-react'
                ...
                {filteredBlogs.map((blog, index) => (
                  ...
                  >
                    <Link href={`/blogs/${blog.slug}`}>
                      <div className="bg-dark-800/50 rounded-xl overflow-hidden border border-dark-700 hover:border-primary-500/30 transition-all duration-300 card-hover h-full flex flex-col">
                        {/* Featured Image */}
                        <div className="relative h-48 bg-gradient-to-br from-primary-900/20 to-accent-900/20 overflow-hidden">
                          ...
                          <div className={`absolute inset-0 bg-gradient-to-br from-primary-500/10 to-accent-500/10 flex items-center justify-center ${blog.featuredImage ? 'hidden' : ''}`}>
                            <BookOpen className="w-16 h-16 text-primary-400 opacity-50" />
                          </div>
                          <div className="absolute bottom-4 left-4 px-2 py-1 bg-dark-800/80 text-gray-300 text-xs rounded">
                            {blog.readTime} min read
                          </div>
                          <div className="absolute bottom-4 right-4 flex items-center space-x-2 text-gray-300 text-xs">
                            ...
                            >
                              <Heart className={`w-3 h-3 ${checkIfLiked(blog.$id) ? 'fill-current' : ''}`} />
                        ...
                        <div className="p-6 flex-1 flex flex-col">
                          ...
                          <div className="mb-4">
                            <div className="flex flex-wrap gap-2">
                              ...
                              {blog.tags.length > 3 && (
                                <span className="px-2 py-1 bg-gray-600/20 border border-gray-600/30 rounded text-xs text-gray-400">
                                  +{blog.tags.length - 3}
                                </span>
                              )}
                            </div>
